...
describe('runChangesetsFixedVersion failure recovery', () => {
  ...
  it('restores existing package changelog when a later step fails after changeset version generated it', async () => {
    const fileA = join(dir, 'packages', 'pkg-a', 'CHANGELOG.md')
    const syntheticFile = join(dir, '.changeset', 'release.md')
    const originalContent = '# Changelog\n\n- Historical entry.'
    const afterVersion = vi.fn()
    ...
    const config: ReleaseConfig = {
      ...makeReleaseConfig(dir),
      afterVersion,
      versionPackage() {
        throw new Error('normalize package failed')
      },
    }

    await expect(runChangesetsFixedVersion(config, '1.0.0')).rejects.toThrow(
      'normalize package failed',
    )
  ...
  it('removes generated package changelog when a later step fails after changeset version created it', async () => {
    const fileA = join(dir, 'packages', 'pkg-a', 'CHANGELOG.md')
    const syntheticFile = join(dir, '.changeset', 'release.md')
    const afterVersion = vi.fn()
